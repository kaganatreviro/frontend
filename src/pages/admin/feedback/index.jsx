...
const initialQuestions = [
  {
    id: 1,
    question: "Как часто обновляется ваш продукт?",
    answer: "",
    timestamp: new Date("2024-04-01"),
  },
  {
    id: 2,
    question: "Где я могу найти документацию?",
    answer: "Документацию можно найти на нашем сайте.",
    timestamp: new Date("2024-04-02"),
  },
  {
    id: 3,
    question: "Какие функции включены в основной пакет?",
    answer: "",
...
function Feedback() {
  ...
  const handleSave = (values) => {
    const newQuestions = questions.map((q) =>
      q.id === currentQuestion.id ? { ...q, answer: values.answer } : q
    );
    ...
    <div className="admin_feedback">
      ...
      {filteredQuestions.map((question) => (
        <Card
          ...
          extra={question.answer ? "Отвечен" : "Неотвечен"}
          style={{ marginBottom: 16 }}
        >
          <p>{question.question}</p>
          {!!question.answer.length && <p>Ответ: {question.answer}</p>}
          {!question.answer && (
            <Button type="primary" onClick={() => showModal(question)}>
              Ответить
            </Button>
          )}
        </Card>
      ...
      >
        <Form onFinish={handleSave}>
          <Form.Item
            name="answer"
            rules={[
              { required: true, message: "Пожалуйста, введите ваш ответ!" },
            ]}
          >
